...
<style>
/* ── BASE ───────────────────────────────────────────────────────────────────── */
html, body, [data-testid="stAppViewContainer"] { background-color: #ffffff !important; color-scheme: light !important; }
...
/* Radio selected dot — force green (overrides Streamlit cloud default red) */
section[data-testid="stSidebar"] [role="radio"][aria-checked="true"] > div:first-child { border-color: #22b545 !important; background-color: #22b545 !important; }
section[data-testid="stSidebar"] [role="radio"][aria-checked="true"] > div:first-child > div { background-color: #22b545 !important; }
section[data-testid="stSidebar"] [data-baseweb="radio"] [aria-checked="true"] > div { border-color: #22b545 !important; }
section[data-testid="stSidebar"] [data-baseweb="radio"] [aria-checked="true"] > div > div { background-color: #22b545 !important; }

/* ── SIDEBAR COLLAPSED BUTTON (always visible, especially on mobile) ─────────── */
...
[data-testid="stSidebarCollapsedControl"] * { color: #ffffff !important; fill: #ffffff !important; }
[data-testid="stSidebarCollapsedControl"] svg { fill: #ffffff !important; stroke: #ffffff !important; }
[data-testid="stSidebarCollapsedControl"] svg path { fill: #ffffff !important; }
/* Header bar — always white so toggle is visible against it */
header[data-testid="stHeader"] { background-color: #ffffff !important; }

/* ── MULTISELECT — force white background (no black on mobile) ─────────────── */
[data-baseweb="select"] > div { background-color: #ffffff !important; color: #333333 !important; }
[data-baseweb="select"] > div > div { background-color: #ffffff !important; }
...
[data-baseweb="menu"] li { color: #333333 !important; }
[data-baseweb="menu"] li:hover { background-color: #f0faf2 !important; }
.stMultiSelect > div, .stMultiSelect > div > div { background-color: #ffffff !important; }
...
/* ── PLOTLY CHART TEXT — force readable dark on mobile ──────────────────────── */
.js-plotly-plot .plotly .xtick text,
.js-plotly-plot .plotly .ytick text,
.js-plotly-plot .plotly .gtick text,
.js-plotly-plot .plotly .legend text,
.js-plotly-plot .plotly .g-xtitle text,
.js-plotly-plot .plotly .g-ytitle text,
.js-plotly-plot .plotly text { fill: #333333 !important; color: #333333 !important; }
...
@media (max-width: 768px) {
    ...
    /* Force white on filter/select on mobile */
    [data-baseweb="select"] > div,
    [data-baseweb="select"] > div > div,
    .stMultiSelect > div { background-color: #ffffff !important; }
    /* Sidebar toggle — extra visible on phone */
    [data-testid="stSidebarCollapsedControl"] {
        background-color: #22b545 !important;
...
# ── GLOBAL PLOTLY TEMPLATE — readable fonts on all screens incl. mobile ─────────
import plotly.io as pio
pio.templates["farmhub"] = go.layout.Template(
    layout=dict(
        font=dict(family="sans-serif", size=12, color="#333333"),
        xaxis=dict(tickfont=dict(size=11, color="#444444"), title_font=dict(size=12, color="#555555")),
        yaxis=dict(tickfont=dict(size=11, color="#444444"), title_font=dict(size=12, color="#555555")),
        legend=dict(font=dict(size=11, color="#444444")),
    )
)
pio.templates.default = "plotly+farmhub"
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("---")
    with st.expander("View & Export Livestock Data"):
        if not profiles_df.empty:
            st.markdown("**Animal Profiles**")
            prof_export = profiles_df.copy()
            for col in ["dob", "arrival"]:
                if col in prof_export.columns:
                    prof_export[col] = prof_export[col].apply(
                        lambda x: x.strftime("%d %b %Y") if pd.notna(x) else "")
            st.dataframe(prof_export, use_container_width=True, height=220)
            st.download_button("Download Animal Profiles CSV",
                               data=prof_export.to_csv(index=False).encode("utf-8"),
                               file_name="farmhub_animal_profiles.csv", mime="text/csv")
        health_events = events_df[events_df["type"].isin(["Vaccination", "Medication"])] if not events_df.empty else pd.DataFrame()
        if not health_events.empty:
            st.markdown("**Health Events**")
            health_export = health_events[["date", "type", "description", "detail"]].copy()
            health_export["date"] = health_export["date"].dt.strftime("%d %b %Y")
            st.dataframe(health_export, use_container_width=True, height=200)
            st.download_button("Download Health Events CSV",
                               data=health_export.to_csv(index=False).encode("utf-8"),
                               file_name="farmhub_health_events.csv", mime="text/csv")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("---")
    with st.expander("Smoked Fish Batch Log & Export"):
        ...
        fish_csv = smoked_df[["date", "month_label", "qty"]].copy()
        fish_csv["date"] = fish_csv["date"].dt.strftime("%d %b %Y")
        st.download_button("Download Fish Batch Log CSV",
                           data=fish_csv.to_csv(index=False).encode("utf-8"),
                           file_name="farmhub_fish_batches.csv", mime="text/csv")
    with st.expander("Fingerling Stocking Records & Export"):
        if not fingerlings_df.empty:
            fing_display = fingerlings_df.copy()
            fing_display["arrival"] = fing_display["arrival"].apply(
                lambda x: x.strftime("%d %b %Y") if pd.notna(x) else "")
            fing_display = fing_display[["pond", "qty", "breed", "size", "arrival", "location", "transport"]]
            fing_display.columns = ["Pond", "Qty", "Breed", "Size", "Arrival", "Source", "Transport (₦)"]
            st.dataframe(fing_display, use_container_width=True)
            st.download_button("Download Fingerlings CSV",
                               data=fing_display.to_csv(index=False).encode("utf-8"),
                               file_name="farmhub_fingerlings.csv", mime="text/csv")
    ...
    if not sales_df.empty:
        ...
        for _, row in sales_df.iterrows():
            ...
            st.markdown(
                ...
                unsafe_allow_html=True)
        st.markdown("---")
        with st.expander("View & Export Dispatch Data"):
            sales_display = sales_df.copy()
            sales_display["date"] = sales_display["date"].apply(
                lambda x: x.strftime("%d %b %Y") if pd.notna(x) else "")
            sales_display = sales_display[["date", "location", "packs"]].rename(
                columns={"date": "Date", "location": "Location", "packs": "Packs"})
            st.dataframe(sales_display, use_container_width=True, height=250)
            st.download_button("Download Dispatch Log CSV",
                               data=sales_display.to_csv(index=False).encode("utf-8"),
                               file_name="farmhub_dispatch_log.csv", mime="text/csv")
    else:
